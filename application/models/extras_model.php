    ...
    Class extras_model extends CI_Model
    {
        ...
        public function ProximosACobrar()
        {
            $deudores = $this->db->query("SELECT
                ...
                max(if(t_intereses.MES IS NULL,0,t_intereses.MES)) MES,
              ...
              FROM t_solicitudes
                ...
                LEFT JOIN t_intereses USING (ID_SOLICITUD)
            ...
            foreach ($deudores as $deudor)
            {
                $Listdeudores[] =
                    [
                        ...
                        'MES' => $deudor->MES == 0 ? '<b>Ninguno</b>' : (new DateTime($deudor->FECHA_INICIO))->add(new DateInterval('P' . $deudor->MES . 'M'))->format('d/m/Y'),
